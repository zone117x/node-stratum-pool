        ...
        _this.daemon.batchCmd(batchRpcCalls, function(error, results){
            if (error){
                emitErrorLog('getblocktemplate call failed for daemon instance ' +
                    results.instance.index + ' with error ' + JSON.stringify(results));
                callback(results);
            } else {
                var candidate = results[1].response;
                candidate.height = results[0].response.fullHeight + 1;
